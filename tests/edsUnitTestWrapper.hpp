...
{
    return style_replace_non_alpha_numeric( boost::to_upper_copy( style_date_nice() ), '_' );
}
    ...
    std::unique_ptr< UnitTestResultWrapper > getResult()
    {
        ...
        if( m_options.bReport )
        {
...

            {
                ...
                // testFile.open( m_htmlFileName.empty() ? m_strFileName.c_str() : m_htmlFileName.c_str(),
                // std::ifstream::in );
